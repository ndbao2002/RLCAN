...
if __name__ == '__main__':
    ...
    model = TTSR(
        ...
        num_sfe_blocks=args.num_sfe_blocks,
        n_feats=args.n_feats,
        grow_channels=args.grow_channels,
    ...
    if args.lr_rate_lte != args.lr_rate:
        LTE_layer_list = []
        for name,param in model.named_parameters():
            if 'LTE' in name:
                LTE_layer_list.append(name)
        LTE_params = list(map(lambda x: x[1],
                        list(filter(lambda kv: kv[0] in LTE_layer_list, model.named_parameters()))))
        base_params = list(map(lambda x: x[1],
                            list(filter(lambda kv: kv[0] not in LTE_layer_list, model.named_parameters()))))

        optimizer = optim.Adam([
                {"params": LTE_params, "lr": args.lr_rate_lte},
                {"params": base_params},
            ],
            lr=args.lr_rate,
